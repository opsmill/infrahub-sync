"""Service-only adapters for PostgreSQL records and S3-compatible artifacts."""
...
class ServiceStorageStartupError(RuntimeError):
    """Service durable storage could not initialize at process construction."""

    def __init__(self) -> None:
        super().__init__("service durable storage startup failed")
...
def service_product_projection(
    *,
    ...
    """Build the one PostgreSQL/S3 projection used by a service process."""
    ...
    try:
        ...
        raise ServiceStorageStartupError from None
    try:
