...
refuse one. For a locally built
Infrahub image, set `INFRAHUB_DOCKER_IMAGE_DIGEST` to an empty value in
`development/preview.local.env` so Compose uses the local tag without a pin.
