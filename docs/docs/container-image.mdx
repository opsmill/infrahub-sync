...
version, and the exact `linux/amd64` configuration digest to expect. It also
records the image archive and example package checksums for the packet task. The
...
After qualification, restore the exported Linux amd64 image archive from the
candidate image artifact and run `uv run invoke release.packet`. The task checks
the image, bundle, and example package against their recorded SHA-256 values
before writing a tester download. See
[Building a private tester packet](develop/guides/building-a-tester-packet.md)
for the input paths and packet layout.
