...
`.release/artifacts.json`, which the workflow writes from what each upload
returned.

On a pull request both that file and the record are transient. They exist so the
clean-host gate below can be handed this candidate's bytes and told what to
expect of them, and the run deletes the candidate, the kit and the record before
it completes. Retaining a candidate, and binding an approval to the identifiers
such a record carries, belong to a manual run against an exact merged commit and
...
Nothing on this page publishes, and this line has no publication workflow yet.
The legacy route is unchanged: a published release still uploads the
distributions it builds, and no workflow a pull request can start uploads
anything at all.

Retaining a candidate, and approving one for publication, are later units of
work. Until they land, `release.identity`, `release.build`, `release.kit` and
`release.qualify` produce and describe a candidate that the clean-host gate
qualifies and the run then discards.
