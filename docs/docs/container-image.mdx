...
identifiers, digests, and retention an approval is later bound to. Nothing writes
that file on this branch, so running `release.qualify` locally refuses until you
supply one. The manual run that produces it is a later unit.
