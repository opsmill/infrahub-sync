...
`release.qualify` is the validation and recording task prepared for a candidate
workflow, and no workflow in this repository calls it yet. It refuses a candidate
that qualified nothing: it requires an image smoke result for every built
platform and a Compose lifecycle result, and each of those records the exact
image digest it ran against.

It also requires `.release/artifacts.json` — the artifact record its caller
writes from what the service returned for each upload — so the record carries the
identifiers, digests, and retention an approval is later bound to.
...
service is holding under another release. Nothing writes that file on this
branch, so running `release.qualify` locally refuses until you supply one. The
manual run that produces it is a later unit.

## What publishes today

No workflow promotes what these tasks produce. The pull-request workflow that
runs them retains nothing at all: it declares no artifact upload, so a run keeps
no image, layout, distribution or bundle.
...
Two things are deliberately absent. The manual run that builds a retained
candidate and writes `.release/artifacts.json` is a later unit, and a publication
bound to such a candidate is a later unit again, separately approved. Until then
`release.build` produces the two distributions a publication would upload, and
