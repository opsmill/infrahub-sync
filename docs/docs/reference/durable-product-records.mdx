...
| `INFRAHUB_SYNC_S3_ENDPOINT_URL` | Optional exact Boto3-compatible `http` or `https` URL with no userinfo and a DNS/registered-name, valid IPv4, or bracketed valid IPv6 host; an optional valid port is accepted, IPvFuture is not, and the value reaches Boto3 unchanged. |
