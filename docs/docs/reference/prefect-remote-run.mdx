...
- `redis>=4.3,<9` is now declared directly, because `infrahub_sync/utils.py` imports
  diffsync's `RedisStore` unconditionally and therefore needs a redis client in every
...
replaced, because replacing a shorter one turns redaction into a substring shredder over
ordinary message text (`within 6***.0 seconds`). A credential shorter than that — a
...
| `COMPLETED` | `no-change` | `dry-run` (plan) or `applied` (sync) | Destination already matches the source |
