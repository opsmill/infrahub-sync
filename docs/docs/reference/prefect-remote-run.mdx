...
| `branch` | `str` or `null` | `null` | Infrahub branch, forwarded exactly as the CLI `--branch` option. When omitted, the configuration's own branch setting applies, else the server default. |
...
No API façade of our own, no reviewed-plan workflow, no per-stage tasks, no work pools or
workers, no triggers, and no production deployment profile. Concurrency guarantees are
limited to the per-configuration lock the sync engine already applies on a single runner
host: a second run of the same configuration waits for that lock and fails if it times
out, rather than hanging. Beyond it, the preview promises no queuing, ordering, or
overlap policy.
