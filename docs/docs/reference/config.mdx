...
| static | Any | A static value to assign to the field. Any declared value is assigned, including `false`, `0`, `""` and `[]`; omit it or set it to `null` for no static value. Takes precedence over `mapping` when both are declared. | No |
