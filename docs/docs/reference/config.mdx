...
Set `--parallel` on `infrahub-sync sync` to enforce a hard tier boundary:
the engine narrows the destination's `top_level` to one tier at a time so
no tier starts before the previous tier's writes have completed.

If you must override (because the auto-tiered order doesn't match an
adapter quirk), keep the `order:` list — it always wins.
