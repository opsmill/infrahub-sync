...
| `destination_binding` | The destination the plan was computed against, as `{url, branch}`: the resolved endpoint URL — environment variables already applied over settings, and normalized — and the branch. **Never the token.** `apply` compares it against the destination it is about to write to and refuses before any write on a mismatch, so a plan reviewed for staging is not applied to production by accident; `--allow-destination-change` applies anyway. Plans written before this field existed carry no binding, and the comparison is skipped for them. |
...
- `infrahub-sync apply --name X --run-id <id>` — applies the stored `plan/` artifact
  ...
  which is why the comparison sits outside the gate, whose checks are not overridable.
