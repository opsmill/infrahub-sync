...
| `schema_fingerprint` | The destination schema semantics the plan was computed against, as a full SHA-256 digest over only what the configuration consumes: its kinds, their identifiers, their destination human-friendly IDs and uniqueness constraints, its mapped fields, and any mandatory-without-default field on those kinds. Written on plans created through the managed service, where it is required. A managed apply reads the live schema and refuses before any write when the digest differs, so a change that could alter what a retained operation means sends you back to a fresh plan; an unmapped kind, and an unmapped field that is optional or carries a default, leave the digest unchanged and apply normally. |
...
- `infrahub-sync apply --name X --run-id <id>` — applies the stored `plan/` artifact
  ...
  Separately from that gate, `apply` also refuses when the live destination endpoint or branch
  differs from the one recorded in the plan, unless `--allow-destination-change` is passed —
  which is why the comparison sits outside the gate, whose checks cannot be overridden.
