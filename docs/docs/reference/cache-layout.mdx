...
`diff` and `sync` both write `<run_dir>/plan/`, and it is the **only** input
`infrahub-sync apply` reads. It holds two files.
...
artifact and is refused, while a run directory with no `plan/` at all is a run that predates
this format — the reader tells the two apart by construction rather than by guessing.
