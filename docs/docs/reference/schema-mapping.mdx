...
Pick the identifiers your destination actually uses, not the ones the source happens to have. Infrahub keys an address by address plus namespace, while NetBox groups addresses by VRF without requiring them to be unique inside one, so the shipped `examples/netbox_to_infrahub/` mapping turns each VRF name into an IP namespace and identifies on that. Using the VRF as a sync identifier can keep source records distinct even when they map to the same destination object. For example, an address in a VRF named `default` and the same address without a VRF both map to Infrahub's `default` namespace.
...
Because the namespace is keyed by that name, two VRFs sharing a name share one namespace, and a VRF named `default` shares the built-in namespace with every object that has no VRF. Adding this mapping to an Infrahub that already holds imported data does not move the objects a previous sync created; see [Migrate from NetBox or Nautobot](../migrating-from-netbox-or-nautobot.mdx) for how to handle that.
