...
Pick the identifiers your destination actually uses, not the ones the source happens to have. A source such as NetBox keys an address by address plus VRF, while Infrahub keys it by address plus namespace, so the shipped `examples/netbox_to_infrahub/` mapping turns each VRF into an IP namespace and identifies on that. Identify on the VRF instead and two addresses that Infrahub would keep apart end up sharing one object.
