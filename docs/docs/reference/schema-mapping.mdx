...
A plan **records** deletes but `apply` never executes one — it applies the non-deletes, completes
successfully, and reports how many deletes it skipped. Deletes are also only computed when the
destination side ran a full extract; both review depths say which of the two happened. See
[Create and review a plan](../running-a-sync#create-and-review-a-plan).
...
Because the namespace is keyed by that name, this mapping cannot carry two VRFs that share one. The namespace records come from the same VRF list, so the second one raises `ObjectAlreadyExists` and the run stops while loading the source — before a plan exists and before anything is written at the destination. It does not merge the two. A VRF named `default` is a milder form of the same limit: it shares the built-in namespace with every object that has no VRF, so it fails only once one of those objects repeats a prefix or an address. [Key the namespace on the VRF ID instead of its name](#key-the-namespace-on-the-vrf-id-instead-of-its-name) avoids both without touching NetBox. Adding either mapping to an Infrahub that already holds imported data does not move the objects a previous sync created; see [Migrate from NetBox or Nautobot](../migrating-from-netbox-or-nautobot.mdx) for how to handle that.
...
The namespace record and every prefix and address pointing at it derive the same value, so a prefix in NetBox VRF 3 lands in `netbox-vrf-3` whatever that VRF is called, and an overlapping prefix in VRF 4 lands in `netbox-vrf-4`. Objects with no VRF — a null `vrf` or no `vrf` key at all — still go to Infrahub's built-in `default` namespace. Each namespace record keeps the NetBox id of the VRF it came from as its local id, which is what a reference lookup matches on.

A VRF literally named `default` behaves differently here than under the shipped mapping: it is assigned its own `netbox-vrf-<id>` namespace rather than sharing the built-in `default`, so its objects stop colliding with objects that have no VRF.
...
The derived name is only as stable as the NetBox ID behind it. IDs are local to one NetBox instance, so recreating a VRF, restoring it, or reading from a second instance produces a different namespace — as does moving between the two recipes. None of that renames data an earlier sync already wrote; see [Migrate from NetBox or Nautobot](../migrating-from-netbox-or-nautobot.mdx) for what adopting a new namespace identity actually takes.
