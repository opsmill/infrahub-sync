...
Registered V3 workers resolve custom adapters from installed code only. A package may declare a custom adapter on its source side as an installed dotted import target or an installed entry point. The destination must name a bundled adapter that supports the destination role.

### Registered adapter identities

Use one of these installed identities in a registered package:
...
3. **Python entry points**: Installed packages that register entry points
...
The plugin loader can use local filesystem plugins while developing an adapter. `INFRAHUB_SYNC_ADAPTER_PATHS` configures that development behavior; it is not a registered-worker execution path. Install the adapter as a package and use its dotted import target or entry point before declaring it in a V3 configuration package.
