...
Registered V3 workers resolve custom adapters from installed code only. A package may declare a custom adapter on its source side as an installed dotted import target or an installed entry point. The destination remains the built-in Infrahub adapter.
