...
The refusal you receive is the service's fixed envelope — HTTP `422`, code `configs-validation`, family `validation`, no `reason`, and the message `the configuration service refused the request`. The underlying cause is an internal finding, `missing-adapter` at `/configuration/source`, which a client sees through `configs validate` on a stored version rather than through the refused registration. The attempt still leaves durable evidence: its idempotency receipt is reserved and then released, and an audit event is recorded with outcome `unavailable`.
