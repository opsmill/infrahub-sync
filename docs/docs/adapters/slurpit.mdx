...
The Sync service image installs it as part of its `service` package extra; no separate install step is needed.
...
| Name | Description |
| --- | --- |
| `unique_vendors` | Retrieves devices through the synchronous SDK and returns each distinct brand. |
| `unique_device_type` | Retrieves devices through the synchronous SDK and returns each distinct brand, device type, and operating system combination. |
...
| `planning_results`.`<planning_name>` | Retrieves results for the planning whose SDK `name` matches the suffix. For example, `planning_results.routing-table` uses the planning named `routing-table`. |
...
The current `slurpit-sdk` has no `site` API or `get_sites` method, so the example does not map sites.
