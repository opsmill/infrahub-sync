...
| `device.get_devices` or `site.get_sites` | Retrieves devices or sites through the SDK. Other dotted SDK methods are rejected so a data load cannot call an SDK write method. |
