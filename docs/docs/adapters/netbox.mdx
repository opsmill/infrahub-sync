...
Prepare a [configuration package](../configuration-package.mdx) and register it. Then create and review a plan before applying the reviewed plan or running a confirmed sync; [Run a sync](../running-a-sync.mdx) has the canonical procedure. The complete example package is `examples/netbox_to_infrahub/package.yml`.
...
`infrahub-sync` allows defining what gets synchronized from a source to a destination. The shipped `config.yml` is a legacy mapping input that matches the example schema; use `examples/netbox_to_infrahub/package.yml` as the V3 configuration-package envelope for registration.
...
- [Use NetBox or Nautobot with Infrahub](../using-netbox-or-nautobot-with-infrahub.mdx) — running NetBox and Infrahub side by side
- [Migrate from NetBox or Nautobot](../migrating-from-netbox-or-nautobot.mdx) — gradual migration into Infrahub
- [Schema mapping reference](../reference/schema-mapping.mdx) — mapping syntax with NetBox examples
