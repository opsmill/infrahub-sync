...
Prepare a [configuration package](../configuration-package.mdx) and register it. Then create and review a plan before applying the reviewed plan or running a confirmed sync; [Run a sync](../running-a-sync.mdx) has the canonical procedure. The Prometheus mapping example is `examples/prometheus_to_infrahub (node_exporter)/package.yml`.
