...
<Tabs>
  <TabItem value="destination" label="As Destination" default>
  ...
  <TabItem value="source" label="As Source">
...
When Infrahub is used as a destination, the schema mapping defines how source data maps to Infrahub node types. The `name` field in schema_mapping corresponds to the Infrahub schema kind.
...
### Destination identity for writes

For each mapped kind, select and map every field of a complete writable destination key. The key can be the Infrahub human-friendly ID or a declared uniqueness constraint. A partial key, a key with an unmapped field, or a kind with no key is refused before data extraction or planning.

A read-only, server-allocated value or a server-computed value cannot serve as a writable key. For example, if Infrahub allocates `rule_id` and declares `hostname__value` as a separate uniqueness constraint, select and map `hostname`:
...
The read-only `rule_id` may remain in the destination schema. If no complete writable key exists, exclude the kind from the package until the destination schema provides one.
...
Source and owner lineage is applied to newly created relationship edges and to relationship peers newly linked by an update. Rerunning a sync does not backfill lineage onto unchanged, pre-existing relationship edges.
Resource-pool allocations are an exception: their relationship mutation does not include source or owner lineage.
...
- Ensure your Infrahub schema matches the expected node types in schema_mapping
- Run `infrahubctl schema load` to update the schema if needed
- Verify field names match the Infrahub schema attribute names

### No writable destination identity

- An error containing `no complete writable destination convergence identity` names the unusable key components. Check that every component is selected in `identifiers`, mapped in `fields`, and writable in the Infrahub schema.
- If the human-friendly ID uses a read-only or server-computed value, select and map every component of a separate writable uniqueness constraint. A kind with no such key cannot be synced safely.
- After upgrading, review and save a new plan before applying it. Existing saved plans have an older schema fingerprint because writability is now part of the fingerprint.
