...
|---------|----------------------|-------------|
| `url` | `GENERICRESTAPI_URL`, `GENERICRESTAPI_ADDRESS` | API base URL |
| token | `GENERICRESTAPI_TOKEN` | API token/key |
| username | `GENERICRESTAPI_USERNAME` | Basic auth username |
| password | `GENERICRESTAPI_PASSWORD` | Basic auth password |

For direct adapter use, these environment variables take precedence over declared settings. The URL names are checked in the order shown. If you previously relied on bare `URL`, `ADDRESS`, `TOKEN`, `USERNAME`, or `PASSWORD`, rename them with the `GENERICRESTAPI_` prefix; bare names are no longer read. Registered configuration packages use their declared or resolved credential values instead of ambient environment variables.
