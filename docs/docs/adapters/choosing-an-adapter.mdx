...
Prepare a [configuration package](../configuration-package.mdx) and register it. Then create and review a plan before applying the reviewed plan or running a confirmed sync; [Run a sync](../running-a-sync.mdx) has the canonical procedure. The custom-adapter example package is `examples/custom_adapter/package.yml`.
...
A custom adapter is a Python class implementing the same interface as the pre-built adapters. A registered V3 package may declare it only as an installed dotted import target or installed entry point, and only for the source side. See [Local adapters](./local-adapters.mdx) for the registered-worker contract and development-only loader behavior.
