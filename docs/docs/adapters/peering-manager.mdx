...
<ReferenceLink title="Peering Manager Extension" url="https://github.com/opsmill/infrahub/blob/stable/models/examples/ipfabric/ipfabric.yml" openInNewTab />
...
The `source.name` is set to `peeringmanager`, instructing `infrahub-sync` to use the Peering Manager adapter.
