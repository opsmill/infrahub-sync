...
matches it on the human-friendly-ID values its payload carries — and a payload missing
one does not match the existing object: it creates a second one and reports success.
`diff` therefore refuses a create it cannot prove carries every component, naming the
kind and what is missing. A kind that declares no human-friendly ID can be created only
where a uniqueness constraint covers its identifiers.
