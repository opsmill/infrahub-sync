...
like any other peer, and Infrahub's built-in `default` namespace — which no NetBox extract
can produce — is recorded as a literal identity and resolved at apply against the
destination's human-friendly ID for that kind. Separately, `LocationRack` is not
