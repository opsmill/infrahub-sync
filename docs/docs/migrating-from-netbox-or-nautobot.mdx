...
**Nautobot 2.0 and later** have [native IPAM namespaces](https://docs.nautobot.com/projects/core/en/stable/user-guide/core-data-model/ipam/namespace/), added in 2.0.0. Map those directly and carry them into the prefix and address identities, as `examples/nautobot-v2_to_infrahub/` does with its `NautobotNamespace` entry mapped from `ipam.namespaces`. Do not derive the namespace from the VRF name there: the source already separates the objects, and keying on the VRF name instead would merge two namespaces whose VRFs happen to share one.

Under the VRF-derived pattern the namespace is keyed by that name, so the separation holds only while the names are distinct. Two VRFs sharing a name resolve to a single namespace, and a VRF named `default` shares the built-in namespace with every object that has no VRF. Check your source for repeated VRF names before you depend on the split. A source with native namespaces is not affected, because its namespaces are carried rather than derived.
...
:::warning What a saved plan will and will not apply
