...
2. IP namespaces (nothing else has to exist first)
3. Rack and site data (depends on locations)
4. Devices (depends on locations, roles, manufacturers, platforms)
5. Interfaces (depends on devices)
6. IP addresses and prefixes (depends on IP namespaces, interfaces, VRFs)
7. VLANs and VLAN groups
...
IP namespaces are on that list because Infrahub identifies a prefix by namespace plus prefix, and an IP address by namespace plus address. Neither NetBox nor Nautobot's NetBox-style model has a namespace object, so the NetBox example creates one Infrahub IP namespace per VRF name and sends objects with no VRF to Infrahub's built-in `default` namespace. Do the same in your own mapping if your source can hold the same prefix in more than one VRF — otherwise those prefixes all collapse into one Infrahub object and overwrite each other.

The namespace is keyed by that name, so the separation holds only while the names are distinct. Two VRFs sharing a name resolve to a single namespace, and a VRF named `default` shares the built-in namespace with every object that has no VRF. Check your source for repeated VRF names before you depend on the split.
...
The default `SKIP_UNMATCHED_DST` flag leaves those older records in place. Run `infrahub-sync diff` and sync into a branch first, then audit the old objects and the relationships pointing at them before retiring or re-pointing any of them. Infrahub Sync does not migrate them for you, and wiping the destination is not a substitute: it discards Infrahub-only data the sync never owned.
