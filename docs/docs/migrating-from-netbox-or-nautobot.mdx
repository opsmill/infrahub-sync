...
**Creates must prove their own key.** A create carries no destination id, so Infrahub
matches it on the human-friendly-ID values the write carries — and a write missing one
does not match the existing object: it creates a second one and reports success. Those
values may come from a mapped field or from a reference to another model, where the
component is read from the referenced peer's own identifiers. `diff` refuses a create
only when a component is named by neither, or when one of them supplies no usable value,
and it names the kind and what is missing. A kind that declares no human-friendly ID can
be created only where its identifiers fully cover one of the destination kind's declared
...
On the current `from-netbox` mapping and schema-library revision this affects
`IpamPrefix` and `IpamIPAddress`, whose identifiers omit the `ip_namespace` component
their destination kinds require: their updates apply, and a fresh create under them is
refused until the mapping or the schema is corrected. Separately, `LocationRack` is not
