...
matches it on what the write carries. For a destination kind with a human-friendly ID the
write needs every component, each with a usable value, taken from a mapped field or from a
reference to another model — where the component is read from the referenced peer's own
identifiers. For a kind that declares none, the model's identifiers must cover at least one
of the destination kind's declared uniqueness constraints. `diff` refuses a create whose
coverage is missing or whose values are unusable, naming the kind and what is missing.
