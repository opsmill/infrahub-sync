...
2. Browse the left navigation for imported objects from the NetBox demo. Depending on the current demo data and the example mapping, you may see objects such as:
   ...
   example, `Data` in group `Site A` becomes `Site A-Data`). And `DcimDevice` also identifies
   a device by name alone, so both `DcimDevice` entries skip devices whose role is
   `patch-panel`: the demo data names several identically across different sites, and patch
   panels are passive components with no data worth syncing in this minimal mapping anyway.
   :::
