...
2. Browse the left navigation for imported objects from the NetBox demo. Depending on the current demo data and the example mapping, you may see objects such as:
   ...
   NetBox has no IP namespace of its own, so the example creates one Infrahub IP namespace per NetBox VRF. A prefix or IP address in a VRF goes to the namespace named after that VRF, and one with no VRF goes to Infrahub's built-in `default` namespace. That matters because Infrahub identifies a prefix by namespace plus prefix, and an IP address by namespace plus address: without the namespace, the same address in two different VRFs would become a single Infrahub object. The example still syncs the VRF itself, so you keep both the VRF relationship and a namespace that keeps overlapping addresses apart. The built-in `default` namespace is the one peer no NetBox extract can produce, so the plan records it as a literal identity and resolves it against Infrahub when the plan is applied. One limitation to know about: a NetBox VRF actually named `default` shares the built-in namespace with everything that has no VRF.
...
Two source objects map to the same identifier. The part after `__` is the IP namespace the object resolved to, so this means two NetBox IP addresses share both an address and a namespace — either both sit in the same VRF, or neither has a VRF and both landed in `default`. Addresses that only look alike because they are in different VRFs no longer collide, so you are not being asked to delete a legitimate object. Find the two rows in NetBox (**IPAM > IP Addresses**, search the address from the message), decide which one is real, and remove or re-assign the other before you re-run the command.
