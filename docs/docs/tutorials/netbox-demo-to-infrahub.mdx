...
export INFRAHUB_SYNC_SERVICE_FLOW_WORKING_DIRECTORY="$PWD"
...
resolves the values from these variables and no token is ever written into the package or
sent to the Sync API.
...
The service worker cannot resolve a credential the package declares. Adapter credentials
belong in the worker environment, never in the package or `config.yml`: the package names
`NETBOX_TOKEN` and `INFRAHUB_API_TOKEN` and the worker reads their values. Export these
