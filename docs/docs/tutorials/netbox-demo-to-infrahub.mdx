...
The `diff` command loads data from NetBox and Infrahub, compares both sides, and prints the planned changes. It also saves a plan artifact under `.infrahub-sync-cache/from-netbox/<run-id>/plan/`, which you can review on its own with `infrahub-sync diff --name from-netbox --directory sync-projects --from-plan <run-id>`.
...
:::warning A second `diff` against this branch needs `--continue-on-error`

Once the sync has written interfaces, running `diff` or `sync` against the same branch again fails while loading the destination. This is a known defect, and it is not specific to NetBox. See [`diff` fails after a sync that wrote interfaces](#diff-fails-after-a-sync-that-wrote-interfaces) for the error and the workaround.
...
Pass `--continue-on-error` to complete the load:

```bash
uv run infrahub-sync diff --name from-netbox --directory sync-projects --branch netbox-import --continue-on-error
```

The unresolvable peer link is logged and skipped rather than aborting the run. The cost is that the affected interface's `bundle` peer is then absent from the extracted destination state, so the plan carries an update for that object on every run even when nothing changed upstream. Review the warnings before relying on the result.

Two things to be clear about:

- **This is not a NetBox-specific defect, and it is not introduced by the saved-plan workflow.** It is present in the released destination adapter, and it affects any configuration in which a mapped kind references a peer kind whose `identifiers` include a relationship — the `nautobot`, `ipfabric`, `slurpit`, and `aci` example configurations all contain that shape. It is a recorded limitation of this release, assigned to later work.
- **`--continue-on-error` only covers this loading failure.** It does not make a run tolerant of everything: a source record that references a peer the source itself cannot resolve still fails the whole run when the plan is derived, by design. Use the flag for this destination-loading case, not as a general "keep going" switch.
