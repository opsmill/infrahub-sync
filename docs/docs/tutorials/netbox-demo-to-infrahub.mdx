...
other project-template commands that run outside Infrahub Sync. Edit those two URLs directly
in `config.yml` before registering the package if either target isn't at the address already
in the file. Only the two credentials (`NETBOX_TOKEN` and `INFRAHUB_API_TOKEN`) come from the
worker's environment, resolved through the `credentials` block at the bottom of `package.yml`.
