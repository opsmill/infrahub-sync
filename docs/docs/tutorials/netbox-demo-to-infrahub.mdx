...
2. Browse the left navigation for imported objects from the NetBox demo. Depending on the current demo data and the example mapping, you may see objects such as:
   ...
   several sites. The mapping prefixes each Infrahub rack name with its site name (for
   example, `Site A-Comms closet`) so those racks remain separate. Device references to
   racks use the corresponding NetBox rack ID and resolve to the renamed rack.
   :::
