...
comparison before any destination data is written, and a source record referencing a peer the
loaded source cannot resolve **fails the whole run** at that point:
...
NOTE  1 delete operation(s) are recorded in this plan and NONE will be executed against the
      destination. An apply of this plan that passes its checks and writes records
...
who applies a plan holding delete records — derived from a full destination load against a
destination holding objects the source no longer has — will meet a non-zero skipped-delete count,
and should not read it as a broken run. (A plan derived from an incremental destination load may
compute no deletes at all.) The skipped identifiers are recorded on the run, so the difference
between what you reviewed and what was written is always readable.
