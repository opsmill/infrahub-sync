...
- `--run-id` — re-use a specific cache run id for the live comparison; useful when you want to overwrite a previous run's plan in place.
...
count, and every delete in the `--detail` listing is marked `(not executed)`:
...
NOTE  1 delete operation(s) are recorded in this plan and NONE will be executed against the
      ...
      1 skipped deletes on the run; each delete record is marked "(not executed)" in the
      --detail listing.
```
...
On completion the run's `run.json` records `applied_operations`, `skipped_delete_operations` and
`skipped_delete_count` in its `summary`, so what the apply did is a recorded value rather than
something you have to infer.

An apply that does **not** complete records the same three keys before it exits. A refusal records
them empty; a destination rejection part-way through records the operations written before it; and
an apply you interrupt with Ctrl-C records what had been written at the moment you stopped it, with
...
INFO    | Applied run 20260518T1430-abc12345: 3 operations applied, 1 deletes skipped
```
...
apply issues the write anyway and warns once per kind that it did:
...
WARNING | Planned write: the mutation rendered for destination kind InfraInterface carries
          ...
          (...). The write was issued anyway. Watch for a duplicate object of kind
...
| The run holds no plan artifact | The run predates the saved plan format. Re-run `diff` to produce a current-format plan. |
