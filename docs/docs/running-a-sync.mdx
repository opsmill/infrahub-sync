...
**This is a behavior change in this release.** Both `diff` and `sync` now derive a plan from the
...
A configuration that syncs today can therefore start failing on this release without the source or
the configuration having changed.
...
# 3. Apply the reviewed plan — no source extraction
...
The mismatch is reported before the destination is contacted. Combined with immutable generations
above, an approval then names the exact bytes it approved rather than a run id that could hold
something else — worth passing whenever the plan is reviewed somewhere other than where it is
applied.
...
destination write: the object is upserted first, and any cardinality-many relationship sets it
...
destination, re-run `diff`, and apply the new plan: re-applying an operation that already
succeeded, in whole or in part, converges on the same object rather than duplicating it.
...
Both lines are logged at `WARNING`, so `--quiet` still shows them: a skipped-delete count is the one
piece of a completed apply an operator must not have to go looking for. A delete-free apply logs its
completion line at `INFO` instead, which is what keeps `--quiet` silent when there is nothing to
disclose.

**This is a designed limitation of this release, not a failure.** Applying deletes is out of scope
here and is assigned to later work. An operator who runs this against a non-pristine destination —
...
component like `site__name__value` rather than a plain attribute — **convergence is not verified in
this release**. The client cannot render that key from a peer supplied as a resolved node id, so the
...
But if it does not, **a duplicate object of that kind is the recorded limitation of this release**,
...
the same objects — is a property this release establishes for kinds whose convergence key is made of
plain attributes. It is **not established** for a kind whose key crosses a relationship. Whether such
a write converges depends on the destination schema's own uniqueness constraints, not on anything the
plan or the apply controls, so it holds for some schemas and not for others and this release does not
tell the two apart. Treat every kind that warns as one where a second apply may duplicate rather than
...
which kinds are affected is knowable before an apply rather than only from the apply's own output.
...
| Unsupported format version | The artifact was written by a version this release does not understand. The remaining checks are not evaluated, and the message says so. Rebuild the plan, or apply it with the version that wrote it. |
...
| An operation's action is unrecognized | The plan holds an operation this release cannot interpret. It is refused while the artifact is read — before any destination write. |
