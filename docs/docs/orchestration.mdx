...
Infrahub Sync provides two Prefect-backed execution surfaces:

- Install the `prefect` extra to serve one direct deployment for plan and confirmed sync
  requests through Prefect's API. Sync output is forwarded into the flow-run log. See the
  [Prefect remote run reference](./reference/prefect-remote-run.mdx) and
  `examples/prefect_remote_run/README.md` in the repository.
- Install the `managed` extra when an automation client needs a Sync-owned authenticated
  HTTP API, durable records and artifacts, reviewed-plan apply, idempotency, cancellation,
  and worker execution. See the
  [managed Sync HTTP API reference](./reference/managed-http-api.mdx).

**When this works well:**

- You already use Prefect for other infrastructure or data workflows.
- A sync needs to be triggered on demand from another system, over an API rather than a shell.
- You want centralized observability across many sync projects.

**Trade-offs:** Prefect itself has to be operated. If you do not already run it, this adds
operational surface area. The direct deployment runs and observes one plan or confirmed
sync; it does not add scheduling, retry, queue, or overlap policy.

Use the managed API for the supported worker-backed run lifecycle. For schedules, retry
policies, triggers, or composing several syncs into a larger workflow, configure Prefect
around either packaged surface or wrap the CLI in a flow you own.
...
- **Logging.** Infrahub Sync emits lifecycle and adapter records through Python logging.
  Forward them to the logging system used by the scheduler. Public Python API lifecycle
  records include structured run, operation, stage, and outcome attributes.
...
- **Failure recovery.** Inspect the run record and destination after a failed write, then
  calculate a fresh plan. A failed operation can have written part of its change. Safe
  re-application is not established for destination kinds whose identity crosses a
  relationship; follow the [run recovery guidance](./running-a-sync.mdx).
...
- **Built-in retry policies.** Configure retries in the scheduling tool. Treat write
  retries according to the [documented recovery and convergence boundaries](./running-a-sync.mdx).
