...
| The nested `<rel>__<attr>__value` filter spelling for peer lookup is unverified offline. **Verified live (AD091): the spelling is right** — the resolver issued `client.filters(kind="InterfaceLag", device__name__value=…, name__value=…)` and matched, on the run recorded in `opsmill-implement-report.md`, which is kept on the tagged record branch `speckit-run/001-plan-artifact-saved-apply` rather than in this archive | Peer resolution would fail wherever a **peer kind's destination HFID crosses a relationship** — five kinds on the qualified path (V30 as corrected by AD091), not the ten identity-bearing-reference entries this row first named, which is a configuration-side figure for a destination-side question | PD-004 fixes the construction rule from the schema's own HFID paths, and AD043's recursive `{peer_kind, identity}` shape is what makes the nested arm constructible without splitting a unique-id; the spelling is asserted by an `integration`-marked test, and a zero match is a loud refusal (FR-014), never a silent drop |
