...
**Feature**: `saved-plan-artifact-and-apply` | **Date**: 2026-07-26 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `dev/specs/001-plan-artifact-saved-apply/spec.md`, itself
derived from the repository-local saved-plan requirements in [spec.md](./spec.md).
...
AD001–AD088 in the spec's Clarifications are treated as settled input throughout. Where this plan had
...
| **AD075** | The replace-set reconciliation gains an explicit **flush** after the loop, because the peer-set editors are purely local and an unflushed reconciliation is discarded. The test observable moves onto **the issued destination write carrying the reconciled peer list**. The flush form is amended by AD085 and again by AD088 |
...
| **AD088** | **Amends AD085's flush form, and corrects its attribution.** No form of a whole-node update is the flush: rendering a node the SDK considers existing emits `<rel>: null` for every **unmapped optional cardinality-one** relationship, so a whole-node flush clears destination fields the plan never mapped — which FR-013's own "MUST NOT touch unmapped destination fields" forbids. The flush becomes a **targeted write**: a hand-built `<kind>Update` carrying `id` plus only the cardinality-many fields being replaced. Pre-initialising the unmapped fields is **rejected** as treating the symptom. Everything AD065/AD075 established stands. **The defect was latent in AD075, not introduced by AD085**: the null goes out under both render modes. The conformance fixture gains an unmapped optional cardinality-one relationship, the harness gains an assertion that the flush names no unmapped field, and the SDK-boundary tripwire is re-pointed from the suppression behaviour to the render behaviour |
...
| V13 | Peer resolution today reads the **loaded** SDK node store (`store.get(key=…, kind=…)`), populated during `model_loader` — exactly the dependency the saved-plan apply path cannot satisfy | `infrahub_sync/adapters/infrahub.py:57-94` (store reads `:78,81`); store populated `:454`, `:501`, `:613` |
...
| V30 | **CORRECTED (AD091) — one row was carrying two different figures, and the smaller one is the one that governs keying.** **(a) Configuration-side, unchanged and still true**: on the qualified configuration **ten schema-mapping entries across nine kinds** declare `identifiers` containing a field that carries a `reference` (`LocationRack.site`, `DcimDeviceType.manufacturer`, `DcimDevice.location` **twice**, `Interface{Physical,Virtual,Lag}.device`, `IpamVLAN.vlan_group`, `IpamPrefix.vrf`, `IpamIPAddress.vrf`) — so a **plan identity** value is sometimes a peer rather than a scalar. That is the one question this figure answers, and it is the question AD043's nested `{peer_kind, identity}` shape exists for. **(b) Destination-side, and this is what decides whether the convergent upsert is keyed and how a peer must be filtered**: the governing fact is the **destination kind's `human_friendly_id`**, read from the destination schema, and it is a *different and smaller* set. Of the **20 kinds** the qualified configuration maps or references (21 entries), **five kinds — five entries — have an HFID that crosses a relationship**: `InterfacePhysical`, `InterfaceVirtual` and `InterfaceLag` (all `['device__name__value', 'name__value']`), `IpamPrefix` (`['ip_namespace__name__value', 'prefix__value']`) and `IpamIPAddress` (`['ip_namespace__name__value', 'address__value']`). The other **fifteen are all-direct** — `['name__value']` in every case but `DcimCircuit` (`['circuit_id__value']`) and `IpamAggregate` (`['prefix__value']`) — and they include `LocationRack`, `DcimDeviceType`, `DcimDevice` and `IpamVLAN`, four of the nine kinds figure (a) counts. **All 20 also declare a uniqueness constraint.** A further split inside the five: only the three interface kinds' plan identities *supply* the crossing component (`identifiers: ["device", "name"]` → the nested device identity carries `name`); `IpamPrefix` (`["prefix", "vrf"]`) and `IpamIPAddress` (`["address", "vrf"]`) supply **no** `ip_namespace` component at all, so for those two the HFID cannot be formed from the plan identity even in principle and AD051's per-component diagnostic **refuses before the write** rather than AD066's gate warning and proceeding | (a) `examples/netbox_to_infrahub/config.yml`, `identifiers:` cross-referenced against `fields[].reference`; enumerated in [research.md](./research.md) PD-004. (b) the live destination schema (`opsmill/schema-library@bgi-schema-library-v2`), queried per AD091's recorded command — `client.schema.all(branch="main")` then `human_friendly_id` for every kind named in `schema_mapping[].name` or `schema_mapping[].fields[].reference` |
...
| V40 | **New (AD075).** `RelationshipManagerSync` has **no `save`**. `add()` and `remove()` only mutate `self.peers` and set `_has_update = True`; neither issues a client call, so a reconciled peer set reaches the destination **only on a later save of the node**. That is how the pre-existing shape behaves too: `update_node` returns the node unsaved and its caller flushes it. The flush is an **update**, not a second `save(allow_upsert=True)`, which would re-render the upsert create; the manager renders the **full** peer list, which is what makes the write a replace. The flush form is neither of those two calls — see V42, which amends the plain-save half of this row, and V43, which replaces the whole-node update with a targeted relationship write | `.venv/…/infrahub_sdk/node/relationship.py`: no `def save` (class at `:238`), `add` `:322-332`, `remove` `:339-357`, `has_update` `:57`, full render `:68-69`; `.venv/…/infrahub_sdk/node/node.py:1810-1811`, `:1526-1536` (dispatch `:1533-1534`), `:1867-1870`, `:352`, `:362`; `infrahub_sync/adapters/infrahub.py:177`, `:625-626` |
...
| V42 | **New (AD085), amending V40.** A **plain** `node.save()` drops a cardinality-many relationship reconciled to the **empty** set, so it is the wrong flush. `save()` dispatches `update()` with `do_full_update` defaulting to `False`, which renders with `exclude_unmodified=True`; `_strip_unmodified`'s **first** loop keeps the emptied manager (a manager defines neither `__bool__` nor `__len__`, so it is always truthy and the `not relationship_property` guard never fires, and the `has_update` arm keeps it), but its **second** loop pops it: `generate_payload_create` wrote `[]` for the same field, so `data[item] == original_data[item]` is `[] == []`, and the pop fires because a relationship manager is not an `Attribute`. The differing-payload path is not where it is lost — `_strip_unmodified_dict` is dispatched only for a `dict` original and a cardinality-many relationship is written as a **list**. `node.update(do_full_update=True)` renders with `exclude_unmodified=False`, so `_strip_unmodified` never runs, the emptied set survives as `[]`, `id` is still rendered, and the mutation is still `f"{kind}Update"`. Non-empty replaces are unaffected. **The remedy in this row is superseded by V43**; the reading of the stripping stands | `.venv/…/infrahub_sdk/node/node.py:1870`, `:290-291`, `:295-296`, `:1872`, first loop `:354-364` (guard `:356`, `has_update` arm `:362`), second loop `:365-370` (`Attribute` guard `:368-370`), dispatch guard `:372`; `.venv/…/infrahub_sdk/schema/__init__.py:179`; empty render `.venv/…/infrahub_sdk/node/relationship.py:68-69` |
| V43 | **New (AD088), replacing V42's remedy.** **No whole-node render can be the flush.** `_generate_input_data` emits `data[<rel>] = None` for every **optional cardinality-one** relationship left uninitialized once `_existing` is `True` — the SDK's own comment says it is there "to allow clearing relationships" — and `save(allow_upsert=True)` marks the node existing via `_process_mutation_result`. So the flush nulls every optional cardinality-one relationship the plan never mapped. **The null is independent of the suppression setting, which is why the defect predates AD085**: with stripping **off** nothing is stripped; with it **on** the first loop's `elif` requires a `RelationshipManagerBase` and its `if` requires a non-optional `RelatedNodeBase`, so an uninitialized optional cardinality-one relationship matches neither, and the second loop never visits the key because an unmapped field is absent from `original_data`. The flush is therefore built by hand: `Mutation(mutation=f"{kind}Update", input_data={"data": {<rel>: manager._generate_input_data(), …, "id": node_id}}, query=node._generate_mutation_query())` issued through `client.execute_graphql` and handed back to `_process_mutation_result` — the same construction `update()` performs, minus the whole-node walk. The peer list is still rendered by the manager, so the replaced fields render byte-identically to V42's remedy, `[]` included | `.venv/…/infrahub_sdk/node/node.py:260-266` (the null), `:1811` (`_existing = True`), `:288-291` (stripping dispatch), first loop `:354-364`, second loop `:365-370`, `update()` `:1867-1888`; `.venv/…/infrahub_sdk/node/relationship.py:68-69`; `.venv/…/infrahub_sdk/graphql/query.py:46-77`; `infrahub_sync/adapters/infrahub.py` `_flush_replaced_relationship_sets` |
...
*GATE: evaluated before Phase 0 and re-evaluated after Phase 1 design. Result: **PASS**, with **three**
recorded and justified tensions — Principle II (idempotency holds only for an all-direct HFID; the
larger of the three, recorded in [Risks](#risks)), Principle III (the write surface exists on one
adapter of nine) and Principle IV (four deliberate broad exception handlers), the latter two recorded
in [Complexity Tracking](#complexity-tracking). Principle V additionally carries a recorded evidence
gap, which is a deferral rather than a tension.*
...
| **II. Sync idempotency & safety** | PASS **with a recorded tension** | Convergence is the point: creates and updates both route through the HFID-keyed upsert (FR-013, V10) — **but that holds only where the destination kind's human-friendly ID is all-direct, and the row no longer claims it holds unconditionally.** Where the HFID crosses a relationship the SDK renders the mutation with neither `id` nor `hfid` (V39, AD051), so a re-apply **creates a duplicate** instead of converging; the qualified path carries **five such kinds — five mapping entries** (V30 as corrected by AD091: `Interface{Physical,Virtual,Lag}`, `IpamPrefix`, `IpamIPAddress`), and of those only the three interface kinds reach this row at all, because `IpamPrefix` and `IpamIPAddress` supply no `ip_namespace` component and are refused earlier by AD051's diagnostic. **The earlier figure here — ten entries across nine kinds — was the count of *plan identities* containing a reference, read from the configuration, and it is not the count that governs keying**; four of those nine kinds (`LocationRack`, `DcimDeviceType`, `DcimDevice`, `IpamVLAN`) are all-direct at the destination and converge normally. The tension is real and narrower than it was recorded as. The limit is proved rather than asserted away: the conformance harness ships the keyedness assertion for such a kind as a `xfail(strict=True)` naming the risk row (`tests/plan/test_apply_conformance.py:434`, AD067), so the day the hole closes the suite says so; the keyedness gate warns once per affected destination kind at apply time (AD066/AD078); and step 3b's per-component assertion refuses *before* the create when a component cannot be accounted for (AD051). Recorded in full in [Risks](#risks) — the nested-HFID row — and closing it needs the deferred live evidence (AD007). SC-002/SC-003 measure re-apply and both crash windows; every divergence between the reviewed set and the applied set is **recorded rather than inferred**: an unresolved peer (FR-014) and a destination rejection (AD027) refuse and fail the run, and a recorded delete — which the saved-plan apply path does not execute *by design* — is recorded as a count and a set of identifiers with an operator-visible warning naming the count, so the run completes `applied` while the difference stays provably knowable (FR-017, AD055). Partial failure is surfaced with the failing operation identifier, never left ambiguous. |
...
| **IV. Type safety & explicit contracts** | PASS **with a recorded tension** | Every new module is fully typed with modern unions; the record types are Pydantic models in the existing `SyncConfig` style; a specific exception hierarchy (`PlanArtifactError` + its named subclasses, see [contracts/plan-reader-api.md](./contracts/plan-reader-api.md)) covers every **designed refusal** — each one names its own remedy, and the base class declares `next_action: str` so a subclass cannot be added without one (AD059). **The taxonomy does not replace every broad catch, and this row no longer claims it does.** Four deliberate broad handlers remain: `except Exception:` at `infrahub_sync/cli.py:686`, and `except BaseException` at `infrahub_sync/cli.py:694`, `infrahub_sync/potenda/__init__.py:613` and `infrahub_sync/plan/writer.py:55`. Each exists to record run state before the exception continues — a `failed` status, a partial `ApplyRecord` (AD062, AD069), a removed temp file — and **all four re-raise unconditionally**, so nothing is swallowed and no traceback is lost. A narrow catch cannot express "any defect outside the taxonomy", which is exactly what the first of them is for, nor reach `KeyboardInterrupt`, which is not an `Exception` at all. That is a carve-out against this principle's "a broad `except Exception:` is prohibited in touched code", recorded in [Complexity Tracking](#complexity-tracking). No `[[tool.ty.overrides]]` block is added; `uv run ty check .` must exit 0. Where the SDK's dynamic surface forces it, a targeted `# ty: ignore[<rule>]` with a TODO is used at the call site, matching `infrahub_sync/potenda/__init__.py:69-70`. |
| **V. Test discipline** | PASS **with a recorded evidence gap** | ~48 tests written alongside the change, parametrized for the verification matrix and the negative cases, atomic and single-purpose. Live-destination evidence is opt-in behind the existing `integration` marker (V28) rather than a new mechanism. The gap, stated rather than implied (AD045): five of the brief's own criteria — DBA-001, DBA-002, DBA-003 and DBA-008 in full, plus the live half of DBA-007 — together with the live half of this specification's derived SC-016, have **no passing evidence at merge time**, so the brief's completion condition — inspectable passing evidence for every criterion — is not met. Two things narrow it. First, a local **rendered-mutation** conformance harness — built against a real node from a committed `NodeSchemaAPI` fixture, not a mock (AD054) — asserts that the mutation input the SDK renders carries `id` or `hfid` — for an all-direct HFID as a requirement, for a relationship-crossing HFID as a **strict expected failure** against the recorded risk (AD067) — that the replace-set enforcement **issues a destination read** for the relationship before reading the peer set it compares against (AD065), and that two applies of one operation render **byte-identical** mutation inputs (AD068). That is exactly the class of defect AD042 was, and the class those deferred criteria are the only other check on. Its earlier form asserted the assembled `data` against a wholly mocked SDK, where two of the three assertions cannot fail for the right reason — so it must not be counted as narrowing anything unless it is built in the AD054 shape as sharpened by AD065, AD067 and AD068. Second, the deferral is recorded here, in [the evidence map](#success-criteria-evidence-map) and in `tasks.md`, so it cannot read as covered. **A second, distinct class of gap, disclosed here rather than left implied by the scoping above**: this principle asks for adapter edge-case tests covering timeouts and 401/403, and the two new surfaces that issue live destination calls during apply — the planned-write surface on the Infrahub adapter and the apply-time peer resolver — have **none**. Their refusal paths *are* covered (a zero-match and a multi-match peer each refuse and dispatch nothing, `tests/adapters/test_infrahub_planned_write.py:953`, `:974`), so what was missing is specifically the transport-failure and auth-failure edges, not the resolution edges. Unlike the six criteria above this was never blocked on a live server. **CLOSED, and no longer a disclosure: T107 owns it and delivered it.** Four offline cases — two surfaces × two edges — parametrized over the SDK's own exceptions (`ServerNotResponsiveError` for a read timeout, `AuthenticationError` for both 401 and 403) assert that the operator gets `OperationApplyFailedError` naming the failing operation, the run, the count that stays written and a next action, with the original exception chained so the library traceback survives. Recording it as "owed" with no owning task was the defect; an owed item with no owner is a disclosure that nothing acts on. |
...
**Post-design re-evaluation**: no gate moved. Phase 1 design added no dependency, no command group and
no configuration surface. It **did** add broad exception handlers — four, all re-raising unconditionally
— which the Principle IV row above states and [Complexity Tracking](#complexity-tracking) justifies; an
earlier version of this paragraph claimed it added none, which was false. The Principle III tension is
unchanged and remains justified below, and the Principle II tension is recorded in [Risks](#risks).
...
├── spec.md                                  # input (AD001–AD088 settled)
├── plan.md                                  # this file
...
- Every error the reader and verifier raise carries a **next action** (AD059), and where the raising site
  ...
  failures: a plan cannot be honestly summarized when the saved-plan apply path cannot interpret its operation vocabulary
  either.
...
- Payload/relationship split: a payload key whose `SchemaMappingField.reference` is set becomes a
  ...
  kinds on the qualified path hit that case — V30(a), the **configuration-side** figure, which is the right
  one here because the nesting is driven by `SchemaMappingField.reference`; the **destination-side** figure
  that governs keying and filtering is different and smaller (AD091). Cardinality-many references are a list ordered canonically by
...
- Deletes carry no payload (FR-028.1) and never enter the diff the write path consumes — which is what
  ...
  qualified path carry a reference inside their identifiers — V30(a), the **configuration-side** figure,
  which is the one this clause needs — so this is the ordinary delete case there.
...
- `delete` raises `SkippedDeleteOperation`. The engine collects these rather than stopping,
  ...
  **`applied`**, not `failed` (AD055). Not executing a delete is a designed limitation of the saved-plan apply path, so
...
- `create` and `update` both build `data` from the payload plus resolved relationship peer ids, run it
  ...
  **warns once per destination kind** and proceeds: the
  convergent write may still key server-side, and refusing would withdraw the qualified path's
  relationship-crossing-HFID kinds — **five kinds, five mapping entries**, of which the three interface kinds
  are the ones that reach this arm (V30 as corrected by AD091) — from what this outcome delivers, which is the
  relationship-bearing capability DBR-013 and DBA-008 require. **The magnitude is smaller than this plan
  first recorded it** (it cited ten entries across nine kinds, a configuration-side figure for a
  destination-side question), and the trade is unchanged in direction: refusing would still decline every
  interface kind on the qualified path, and the live run confirms the render is unkeyed there. And where the destination kind declares
...
- Cardinality-many relationships are then reconciled as an explicit **replace-set** against the saved
  ...
  **The re-read is a mechanism, not an ordering (AD065).** `fetch()` carries its own
  ...
  must force the manager cold — set `initialized` false, then `fetch()`, then read `peer_ids`. "Fetch first"
  is not a mechanism, and the test asserts that a destination **read was issued** for that relationship
  before the peer set was read. **Narrowed to that one mechanism by AD075's invariant**: reading the manager
  off a separately fetched node was offered here as equivalent and is withdrawn, because the reconciliation
  would then land on the fetched node's manager while the flush below writes the node whose manager is still
  the payload-built one — so the update goes out carrying the create payload's peer list, the desired set that
  was never compared against anything. Forcing the manager cold is that same scoped read plus the write-back the invariant
  needs: `fetch()` assigns the peers it reads back onto the manager it was called on
  (`.venv/…/infrahub_sdk/node/relationship.py:290-299`).
  ...
  **no `save`**: `add()` and `remove()` only mutate the in-memory peer list and set an update flag, issuing
  ...
  (V40). The sequence therefore ends with an explicit flush after the
  loop over every cardinality-many relationship, once, not one write per relationship, **on the same node
  object the reconciliation ran against; the node that is updated must be the node whose manager was
  reconciled**. An update, **not** `save(allow_upsert=True)`, which would re-render the upsert create
  instead — and **not any whole-node update either (AD088)**, because rendering the whole node emits
  `<rel>: null` for every unmapped **optional cardinality-one** relationship of a node the SDK considers
  existing, so a whole-node flush clears destination fields the plan never mapped. That null is independent
  of the unmodified-field stripping, which is why it predates AD085: with the stripping off nothing is
  stripped, and with it on the field survives both loops anyway. So the flush is a **targeted write** —
  `_flush_replaced_relationship_sets` builds `<kind>Update` by hand with `id` plus only the replaced
  relationship fields, the manager still rendering each peer list (which is what makes the write a replace,
  and what carries `peers: []` for an emptied set), and issues it the way `update()` does (V40, V42, V43).
  AD085's reading stands and its remedy does not: an emptied set now survives because it is written
  explicitly, not because a suppression setting was switched off. This is the same shape as the pre-existing code, whose flush lives in its *caller*
...
reading the peer set it compares against (AD065), that the reconciled peer set was then **issued to the
destination**: the step 7e flush — the targeted relationship write on the reconciled node — rendered as
an **update** carrying that peer list, not as a second upsert, and carrying it for an **emptied** set too
(AD075, AD085), that the flush names **no destination field the operation did not map**, against a fixture
kind that declares an unmapped optional cardinality-one relationship for it to name (AD088), and that two applies of one operation render
...
- The renderer carries two obligations the reader deliberately does not (AD056, AD058): it states the
  ...
  delete will be executed by the saved-plan apply path; and it turns an empty `operations(kind=…)` result for a
...
**Five of the six now have live passing evidence; the sixth is refused by the destination schema (AD091).**
A live destination became reachable (Infrahub on `main` from `opsmill/schema-library@bgi-schema-library-v2`,
source `https://demo.netbox.dev`) and the Phase H module was run against it after the slice was widened to
carry a relationship-crossing convergence key. Verbatim: `7 passed, 1 error in 70.74s`.

| Criterion | Live outcome |
|---|---|
| SC-001 | **Passing** — `test_applying_a_stored_plan_runs_no_extraction` |
| SC-002 | **Passing on a bounded slice** — `test_re_applying_an_identical_plan_converges`. The caveat below travels with this claim (AD080's precedent) |
| SC-003 | **Passing on a bounded slice** — `test_the_write_class_conformance_matrix` for all three write classes. Same caveat |
| SC-007 (live half) | **Passing** — `test_a_delete_bearing_plan_applies_and_records_the_skipped_deletes` |
| SC-008 | **Passing** — `test_relationship_peer_sets_match_the_plan`, including the assertion that the resolver issued `client.filters(kind="InterfaceLag", device__name__value=…, name__value=…)`, so PD-004's nested spelling is verified rather than declared |
| SC-016 (live half) | **Not produced — refused as a fixture precondition, and it cannot be satisfied on this destination schema.** Seeding a genuinely ambiguous peer needs a referenced kind whose uniqueness constraints do not cover the components the resolver filters on, and **every one of the 20 kinds this configuration touches declares one that does** (V30). The destination answered the clone with `Violates uniqueness constraint 'device-name'`, HTTP 422. Per **AD092** `test_an_ambiguous_peer_refuses_the_operation` states that as a precondition checked against the destination schema and **skips with the reason** instead of erroring in setup; it runs, unchanged, against any schema whose referenced kinds leave a filtered component free. The offline half (zero-match and multi-match refusal messages) is unaffected and still passes. Not weakened, not mocked |

**The scope caveat that travels with SC-002 and SC-003 (AD080's precedent).** Both ran against a
**bounded** live slice, not the qualified path in full: ten of the configuration's schema-mapping entries
(the reference closure of `InterfacePhysical`), and within those, four entries narrowed by a source-side
filter to one site or one device (`ADDED_FILTERS` — `LocationRack`, `DcimDevice`, `InterfaceLag`,
`InterfacePhysical`). Two of those filters are there for **size**; the `LocationRack` one is there
**because `LocationRack` is not convergent on the qualified path at all** — the Material row in
[Risks](#risks) records why (the destination's key is the rack name alone while the plan identity is
name-plus-site, so thirteen identically named demo racks collapse onto one object, and the churn cascades
through every identity that nests a rack). So "SC-002 passing" means *convergence held on a slice from
which the known non-convergent kind was filtered out*, and it must not be read as convergence across the
qualified path. Nothing was weakened to obtain it — the filters are the documented fixture bounding, not a
loosened assertion — and the excluded kind is disclosed rather than dropped: it is a Material risk reported
to root, with a plan-time detector (a third arm on `warn_missing_convergence_key`) named there and
deliberately not built here. AD080 set the precedent that a caveat of this kind travels with the claim
rather than living only in the risk table.

**What SC-002 and SC-003 settle about the Material nested-HFID risk.** They ran on
`InterfacePhysical`, whose destination HFID is `['device__name__value', 'name__value']` — the exact shape
the risk is about. The keyedness gate fired as designed, once per kind per apply, seventeen times across
the module: *"the mutation rendered for destination kind InterfacePhysical carries neither 'id' nor 'hfid'
because the kind's convergence key crosses a relationship (device__name__value, name__value) … The write
was issued anyway."* And the writes **converged anyway** — two applies of the identical plan left the
same counts and the same objects, and both crash windows recovered to the clean state. So on this
destination the "convergent write may still key server-side" clause is **confirmed**: Infrahub resolves
the upsert on its own `device-name` uniqueness constraint. The risk is therefore narrower again than the
corrected V30 makes it: it bites only a relationship-crossing key with **no** covering uniqueness
constraint, and this configuration has no such kind. AD067's strict `xfail` in the offline harness is
untouched by this and did not xpass — it asserts the *render* carries an identifier, and the render still
does not; it is not parameterised by the live slice.
...
| *(no SC — offline conformance, AD045a as rebuilt by AD054 and sharpened by AD065/AD067/AD068/AD075/AD088)* | **Rendered-mutation** conformance against a real `InfrahubNodeSync` built from a **committed `NodeSchemaAPI` fixture**: the rendered mutation input carries `id` or `hfid` (`.venv/…/infrahub_sdk/node/node.py:295-298`, `:1843-1846`) — asserted outright for an all-direct HFID, and as a `xfail(strict=True)` for a relationship-crossing HFID, which cannot render keyed today (AD067); the replace-set reconciliation **issued a destination read** for the relationship before reading the peer set it compares against (AD065); the reconciled peer set was **issued to the destination** by the flush — the targeted relationship write on the reconciled node, rendering as an **update** carrying that peer list rather than a second upsert, for an **emptied** set as well as a non-empty one (AD075, AD085); the flush names **no unmapped destination field**, against a fixture kind carrying an unmapped optional cardinality-one relationship (AD088); two applies of one operation render **byte-identical** inputs (AD068). Not a criterion of its own — it is the offline half of the assurance SC-002, SC-003 and SC-008 carry, so an AD042-class defect is caught without waiting for the deferred live run. Its earlier form asserted the assembled `data` against a wholly mocked SDK, where two of the three assertions cannot fail for the right reason | local (real node, committed schema fixture) |
...
| **Principle IV** — four deliberate broad exception handlers in touched code, against "a broad `except Exception:` is prohibited": `except Exception:` at `infrahub_sync/cli.py:686` and `except BaseException` at `infrahub_sync/cli.py:694`, `infrahub_sync/potenda/__init__.py:613`, `infrahub_sync/plan/writer.py:55` | Each one's whole job is to leave the record truthful for a case the taxonomy cannot name. `cli.py:686` is *defined* as "outside the taxonomy" — a defect, which no narrow type enumerates — and records `failed` before letting the traceback through. The three `BaseException` arms have to reach `KeyboardInterrupt`, which is not an `Exception` at all: without them a Ctrl-C on a long apply leaves the sidecar at `running` with the empty summary it was saved with, claiming nothing was attempted (AD062), the partial `ApplyRecord` never reaches the CLI (AD069), and `writer.py` leaves its temp file behind. **All four re-raise unconditionally**, so the hazard the prohibition exists to prevent — a swallowed error — cannot arise here | Two alternatives were rejected. **Narrowing the catches** is not expressible: the taxonomy is closed by design and these arms are for what falls outside it, and no `Exception` subclass covers an interrupt. **A targeted lint suppression** carrying the rationale — the shape used at `infrahub_sync/adapters/slurpitsync.py:43` and `:215` — is unavailable, and was verified so rather than assumed: ruff's `BLE001` and pylint's `W0718` both exempt a handler that re-raises, so neither fires at any of the four sites, and `RUF100` (live under `select = ["ALL"]`) then flags an added `# noqa: BLE001` as an unused directive, failing the ruff stage. A suppression would therefore assert compliance in a form the gate rejects; the carve-out is recorded here instead |
...
|---|---|---|
| `save(allow_upsert=True)` may merge rather than replace a cardinality-many relationship set; unverifiable without a live server (AD007) | SC-008 fails; relationships drift | PD-005 makes the replace-set explicit after the upsert, **re-reading the destination's peer set before comparing** (AD054), so the server's own semantics do not decide the outcome. The re-read is the load-bearing part: the pre-existing code reads the peer set before loading it (corrected V12) and a locally built node reports the desired set as its existing set (V12a), so without the re-read the reconciliation is a guaranteed no-op that passes against a mock and removes nothing against a server. **And the re-read must be a mechanism, not an ordering (AD065)**: the SDK's `fetch()` self-guards on `initialized`, so an enforcement that merely calls it first still reads nothing — the manager is forced cold (the one mechanism left standing under AD075's invariant, since the node saved must be the node whose manager was reconciled), and the test asserts a read was issued. **And the reconciliation must be flushed (AD075, AD085, form settled at AD088)**: the peer-set editors issue no client call of their own (V40), so the enforcement ends with one write after the loop — a **targeted relationship write**, a hand-built `f"{kind}Update"` carrying `id` plus only the cardinality-many fields being replaced (V43). Neither withdrawn form will do: a plain `node.save()` strips an emptied peer set back out (V42), and **no whole-node update works either**, because re-rendering a node the SDK considers existing emits `<rel>: null` for every unmapped optional cardinality-one relationship, which FR-013 forbids — the null goes out under both render modes, so no flag on `update()` avoids it (V43). The test asserts the **issued destination write carrying the reconciled peer list** rather than the in-memory peer set — without that, the enforcement is a no-op in exactly the merge case this row is about — and asserts the flush's **field set**, so a return to any whole-node form fails. The enforcement is new code on the planned-write path; `update_node` is untouched (AD070) |
| The nested `<rel>__<attr>__value` filter spelling for peer lookup is unverified offline. **Verified live (AD091): the spelling is right** — the resolver issued `client.filters(kind="InterfaceLag", device__name__value=…, name__value=…)` and matched; the supporting implementation record is retained outside this archive | Peer resolution would fail wherever a **peer kind's destination HFID crosses a relationship** — five kinds on the qualified path (V30 as corrected by AD091), not the ten identity-bearing-reference entries this row first named, which is a configuration-side figure for a destination-side question | PD-004 fixes the construction rule from the schema's own HFID paths, and AD043's recursive `{peer_kind, identity}` shape is what makes the nested arm constructible without splitting a unique-id; the spelling is asserted by an `integration`-marked test, and a zero match is a loud refusal (FR-014), never a silent drop |
...
| An apply that skips deletes records `applied`, which the incremental path's success set already contains (`infrahub_sync/cache/incremental.py:24`) | A delete-bearing apply counts as a successful prior run for a later warm start | Accepted and recorded rather than mitigated (AD055): the apply did succeed at everything the saved-plan apply path executes, and adding a distinct state to say otherwise is the compatibility change AD010 declines. The recorded skip count is what a later reader uses to tell the two cases apart |
| **NEW, found by the live run (AD091). `LocationRack` is not convergent on the qualified path against this destination schema, because the destination's convergence key is *coarser* than the plan identity.** `LocationRack` declares `human_friendly_id: ['name__value']` and `uniqueness_constraints: [['name__value']]` — keyed on the rack name alone — while the configuration's identity is `identifiers: ["name", "site"]`. Thirteen NetBox demo racks are named `Comms closet`, one per site, so thirteen distinct plan identities converge onto one destination object whose `site` is whichever operation wrote last | Every re-derived plan reports a create **and** a delete for the same rack, forever, and the churn cascades through every identity that nests it — `DcimDevice.location`, then `InterfaceLag` / `InterfacePhysical` — so no kind in that chain can be seeded out of a plan. Observed directly: an unbounded slice produced 38 `DcimDevice` creates alongside 36 `DcimDevice` deletes on a destination the seed had just converged | **Not mitigated, and specifically not detected by FR-024.** `warn_missing_convergence_key` tests `constraint <= identity` — "would the destination refuse a duplicate keyed on what the plan supplies" — and here the constraint *is* a subset of the identity, so both of its arms stay silent. The hazard runs the other way: the destination's key is strictly coarser than the plan identity, so distinct plan identities collapse rather than duplicate. A third arm detecting `identity ⊄ any constraint` would catch it; that is new scope and is **not** added here. The Phase H fixture bounds both rack-bearing entries to one site so the criteria it measures are measurable at all, and records why at `ADDED_FILTERS`. **Material — reported to root** |
| **NEW, found by the live run (AD091). The destination *extract* cannot rebuild a peer whose own identifiers include a relationship, so `diff` on the qualified path fails once `InterfacePhysical` exists at the destination.** `resolve_peer_node` re-fetches a peer only when `_node_has_complete_attributes` is false, and that predicate walks **attributes** only (`infrahub_sync/adapters/infrahub.py:59-67`, gate at `:99-100`) — so a peer node that is attribute-complete but carries no relationship data is never re-fetched, `infrahub_node_to_diffsync` skips its relationship for want of `rel.id`, and `_resolve_peer_unique_id` raises `PeerIdentifierError` naming the missing identifier | Re-running the Phase H module against a destination it has already written to fails in the fixture's own `_plan_run`: `Cannot build unique_id for peer InterfaceLag[…] (relationship InterfacePhysical.bundle …): missing identifier key(s) ['device']`. Generalised, `infrahub-sync diff` against the qualified path fails after the first successful sync of `InterfacePhysical`. It is **not** on the apply path — a saved-plan apply re-extracts nothing (FR-012) — so nothing this outcome delivers is affected | **Pre-existing and out of scope here.** Both the predicate and the gate are byte-identical to `main`; the defect is on the destination-extract path shared with the live `sync` command, which AD070 puts off limits for this delivery. Recorded for a later outcome rather than repaired. Consequence for this run's evidence, stated plainly: the seven passing results were obtained against a destination holding no `InterfacePhysical`, and reproducing them requires clearing that kind first. **Material — reported to root** |
