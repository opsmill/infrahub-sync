...
| Non-zero delete count | `summary()` carries `deletes_not_executed`; the renderer must annotate it inline in **both** depths, stating no delete will be executed against the destination by the saved-plan apply path | FR-006, FR-017, AD055, AD056 |
...
| `SourcePeerUnresolvedError` | **Two conditions, and the message routes each to its own remedy (AD082).** (a) **Absent** — a relationship peer is not in the **loaded source store** while the plan is derived. (b) **Ambiguous** — the bounded kind probe (AD050) finds the peer's unique-id in **more than one** candidate model bucket, so its kind cannot be established (FR-030) | the referring operation, the peer kind or the candidate kinds tried, and the peer identity — **and which of the two conditions applies** | **(a) absent**: add the peer's kind to the configuration so it is loaded, or remove the relationship from the mapping. **(b) ambiguous**: disambiguate the field's `reference` across the schema-mapping entries that declare the owning kind, so one candidate remains. The absent-case remedy is **wrong for the ambiguous case** — nothing is missing there, the same unique-id resolved in two buckets — and routing an operator at a condition they do not have is the dead end AD059 exists to remove, which is why the arms are separated rather than left to be inferred from the candidate list. Both are kept textually distinct from `PeerNotFoundError`, whose remedy is a **destination**-side one and fixes nothing here (AD071, AD082) |
