...
| F7 (contract §1 PyPI install line vs T034) | Nit | **CLOSED** | contracts/prefect-flow.md §1 now leads with the preview-accurate repo-checkout form (`pip install -e '.[prefect]'`), names T034's binding rule as the reason, and demotes the PyPI form to "post-publication shape only … must not appear in the example README". Consistent with T034 and with the flow-module docstring requirement in §2/T014. |
...
### F11 — X7's "breaking contract change" wording makes a preview log line a permanent compatibility surface
...
- **Exact divergence**: the brief's owned shared contract is the execution request, `RunResult`, and the two failure classes; the summary line is a preview observation mechanism. Declaring its format change "breaking" without a scope qualifier creates a durable commitment future briefs inherit alongside the brief-owned contract, though the surrounding text ("this preview's contract") suggests preview scope. No backlog leakage (B-001's REST resource model is not introduced).
- **Disposition**: **Artifact defect** (wording). Qualify the sentence ("a breaking change for consumers of this preview; a future API brief supersedes it via the owned contract's extend-not-fork rule") or list the line's contractual status as a gate-packet awareness item.
...
| F7 | — | CLOSED | Contract §1 now leads with the repo-checkout install; PyPI form demoted to post-publication-only and banned from the README. |
...
| F11 | Nit | Artifact defect | Scope-qualify X7's "breaking contract change" sentence so a preview log line does not read as a permanent compatibility surface. |
