...
- [ ] T011 Extend `tests/test_execution_surface.py` (part 2 — refusals and lifecycles): parametrized SC-004 negative set for `sync_name` (`"nope"` unknown, `"../custom-example"`, an absolute path, `"a/b"` separator, `"--help"` flag-like, `"$(touch ...)"` shell-metacharacter/command-substitution) each refused as `RunValidationError` with filesystem spies proving no read outside the configured directory and subprocess spies (patched `subprocess.Popen`/`os.system`) proving no subprocess start; one invalid-content negative: a `config.yml` whose `name` matches but whose content is invalid (bad YAML / pydantic-validation failure) → `RunValidationError` naming the logical name and never including file contents; unconfirmed `operation="sync"` refused BEFORE adapter construction (spy `potenda_factory` never called) with the required message; pipeline-lock contention (held lock, shortened timeout) → `RunExecutionError` naming sync and timeout, bounded not hanging; one execution-failure case: the factory raising `ValueError` for a missing credential (e.g. unset `INFRAHUB_API_TOKEN`) → sanitized `RunExecutionError` whose message names the missing input (the variable/setting name, never any value); a `branch` passthrough assertion: an explicit `branch` value passed to `execute_run`/`run_remote_request` reaches the spy `potenda_factory` unchanged, and omitting it yields the CLI default (`None`); successful `plan` via a fake `potenda_factory` fixture → `status in {"planned","no-change"}`, `run.json` `mode="diff"`/`dry-run`, `plan.parquet` present, `RunResult` fields correct. Depends on T007, T008, T010 (same file). Trace: DBA-006, DBA-007, DBA-010; SC-004, SC-008; DBR-003 (branch passthrough), DBR-004, DBR-005, DBR-008.
...
- [ ] T025 [US4] Refactor `diff_cmd` in `infrahub_sync/cli.py` to delegate to `execution.execute_run`: keep the existing `get_instance` resolution (name OR config_file — CLI-only flexibility) and `adapter_path` merging in the command body; call `execute_run(sync_instance, operation="plan", confirm_writes=False, branch=..., show_progress=..., verbosity=..., run_id=..., concurrent_load=..., full_extract=..., potenda_factory=get_potenda_from_instance)` passing the CLI module-global factory (keeps existing patches on `infrahub_sync.cli.get_potenda_from_instance` effective); map surface errors precisely by origin: `RunValidationError`, and `RunExecutionError` wrapping the factory `ValueError` (`__cause__` is `ValueError`), → `print_error_and_abort` with today's exact wording ("Failed to initialize the Sync Instance: ..." for the factory case); `RunExecutionError` wrapping a factory `ImportError` (`__cause__` is `ImportError`) → re-raise the original `ImportError` unchanged, preserving today's uncaught-traceback behavior (the CLI at `9edc1bc` catches only `ValueError`); exit codes and output identical to the current CLI at `9edc1bc` (DBR-009); log lines, `run.json` contents, and exit codes byte-identical to today. Depends on T007. Trace: DBR-007, DBR-009; DBA-009; contracts/execution-surface.md "Caller obligations".
- [ ] T026 [US4] Refactor the serial branch of `sync_cmd` in `infrahub_sync/cli.py` (the `else` branch taken by `--no-parallel`, or `--parallel` with explicit `order:`) to delegate to `execute_run(sync_instance, operation="sync", confirm_writes=True, print_diff=<--diff>, allow_rowcount_drop=..., continue_on_error=..., ...)` — the explicit human CLI invocation IS the confirmation; the `--parallel ignored` warning and the parallel `sync_in_tiers` branch remain in `cli.py` untouched (spec Out of Scope). Same error-mapping and behavior-preservation obligations as T025 (including the ValueError-origin → `print_error_and_abort` wording and the ImportError-origin re-raise for traceback parity). Depends on T025 (same file). Trace: DBR-007, DBR-009; DBA-009; spec In Scope / Out of Scope.
