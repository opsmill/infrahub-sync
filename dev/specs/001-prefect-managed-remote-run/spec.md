...
- **R-1 (first committed task)**: Plain `uv sync` does not install development tooling (dev tools live in the `dev` extra), so `uv run pytest -q` fails in a fresh worktree. Update the setup and workflow commands in `AGENTS.md` from `uv sync` to `uv sync --extra dev`, committed on its own as the first task of the feature branch. **Superseded in effect by the C1 review remediation (commit `730dc0d`): `AGENTS.md:18,26` now reads `uv sync --extra dev --extra prefect`, and that is the command every fresh worktree/venv must use.** Both extras are required for the gates to mean anything: without `--extra prefect`, `ty` reports 13 diagnostics (6 `unresolved-import` in `orchestration/`) because it cannot see the new package at all, and the flow suite silently skips on `importorskip("prefect")` instead of running. R-1's mandated action and its isolated commit 1 are unchanged — only the final documented command is broader than R-1's literal text.
...
- **Clarified (Session 2026-07-30; RATIFIED — D001, Blake Ellis, 2026-07-30)**: The "canonical plan fingerprint" (DBA-009) is fixed to the definition under Key Entities — a SHA-256 digest over the sorted, canonically serialized plan rows, computed by one shared helper for both the CLI and remote comparison.
- **Clarified (Session 2026-07-30; RATIFIED — D002, Blake Ellis, 2026-07-30)**: The server-configured configuration directory (DBR-005) is supplied through the required `INFRAHUB_SYNC_CONFIG_DIRECTORY` environment variable read at serve start — not through remote parameters — consistent with DBR-006; missing or invalid values fail at serve start, not per-run.
...
- **Dependency (available)**: Prefect 3.8.1 — RATIFIED (D005, option D, Blake Ellis, 2026-07-30): the brief's 3.7.2 pin is unsatisfiable while the base requests `diffsync[redis]` (redis <5.0 via that extra vs >=5 via pydocket in prefect>=3.6); the ratified repair declares `redis` directly in the base with a permissive floor (`diffsync>=2.1,<3.0` + `redis>=4.3,<9`) so the extra can pin the current latest `prefect==3.8.1` — flow, served deployment, REST run creation, state, and logging behavior. VAL-6 and VAL-12 already exercised Prefect locally (supporting evidence only); this feature's Phase 0 probes (research.md) verified install, served-deployment REST runs, log bridging, and parameter validation against 3.5.0, and the gate re-verified the same four load-bearing behaviors directly against 3.8.1 (served-deployment REST run → COMPLETED; run-scoped stdlib-logging bridging retrievable through `POST /api/logs/filter` with levels preserved; invalid `operation` → HTTP 409 with no flow run created; full test suite 111 passed / 2 skipped). Accepted risk: 3.8.1 was released the same day and therefore has zero field exposure — mitigated by those direct behavioral probes rather than by elapsed time.
