...
4. **The conformance harness against a mock (T081, T046, T051) — AD054.** Two assertions cannot fail
   ...
   **rendered mutation**, not of the assembled `data`: by the time `data` is complete a
   ...
   (`.venv/…/infrahub_sdk/node/node.py:295-298`, upsert render `:1843-1846`). **(b)** The replace-set
   ...
   guaranteed no-op that passes against a mock and removes nothing against a server. **(c)** A mock holds
   no destination state, so "two applies produce one create" must be asserted at the **mutation** — one
   `create` invocation with `allow_upsert=True` — not at object counts.
...
- [ ] T003 [P] **[AD059]** Implement the `PlanArtifactError` hierarchy in `infrahub_sync/plan/errors.py`: `PlanFormatV1Error`, `PlanArtifactTornError`, `PlanFormatVersionError`, `PlanArtifactUnreadableError`, `UnknownPlanKindError`, `UnsupportedOperationActionError`, `DuplicateOperationIdError`, `UnserializablePayloadValueError`, `PeerNotFoundError`, `PeerAmbiguousError`, `PlanVerificationError` — [contracts/plan-reader-api.md](./contracts/plan-reader-api.md), Constitution IV. **`PlanArtifactError` declares `next_action: str` on the base class**, required and non-empty, so a subclass cannot be added without one: AD036 attached the next-action obligation to *refusals* only, which is why nine failures named a cause and stopped, and AD059 extends it to the whole taxonomy. `UnsupportedOperationActionError` is new and carries FR-017's genuinely-unsupported class (AD055).
  **Done when**: all eleven import, each derives from `PlanArtifactError`, **constructing any of them
...
- [ ] T026 [P] **[AD056 / AD058]** Write `tests/plan/test_review.py` — summary reports a count per action and a count per kind; per-object detail carries at least operation identifier, action, destination kind and destination identity; `kind` narrows; a zero-operation plan produces a summary that **states** the plan has no operations; a plan whose checksum fails is still rendered with `checksum_ok` false and a note; no `run.json` is written by any review call. **The AD058 split, asserted as two distinct cases that must not be merged**: a kind the configuration **declares** with no operations in the plan returns `[]` and does **not** raise; a kind the configuration does **not** declare raises `UnknownPlanKindError` naming the kind and listing the kinds the plan holds. A test that raises for both would pass against the old, wrong behavior. **The AD056 fields**: `summary()` carries `delete_operations_computed` matching the manifest and `deletes_not_executed` matching the plan's delete count, asserted against a `true`/non-zero fixture and a `false`/zero fixture — FR-006, FR-022, FR-029, AD031, AD056, AD058.
...
- [ ] T042 **[AD038 groundwork / AD054]** [US5] Extract the cardinality-many reconciliation from the **module-level** `update_node` function (`infrahub_sync/adapters/infrahub.py:97`, body `:149-175`) — it is **not** a method on `InfrahubAdapter`, whose class body only begins at `:277` (V35) — into `_replace_relationship_set(node, rel_name, peer_ids)`. **The extraction is deliberately NOT ordering-preserving, and that is the point of the task (AD054).** Today's code reads `attr_manager.peer_ids` at `:151` and only calls `fetch()` at `:168-169`, so it compares the desired set against an unloaded one: `existing_only` comes back empty and it **adds without removing**. It is not a replace-set, and corrected code fact V12 says so. The helper must **fetch first, then read `peer_ids`, then `compare_lists`, then remove `existing_only` and add `new_only`**. Correcting the ordering here rather than only on the new caller is required because the helper is shared: it cannot be a replace-set on one path and additive on the other, which is exactly the shape a defect hides in — FR-013, AD038, AD054 (corrected V12).
  **Done when**: `update_node` calls the helper; `tests/adapters/` passes; and a **new** test asserts the
  re-read explicitly — a manager whose destination peer set differs from the desired one has its
  `existing_only` peers **removed**, which the pre-extraction code did not do. That test must fail against
  the pre-extraction ordering, otherwise the correction is not proven.
...
- [ ] T046 **[AD038 / AD054]** [US5] After the upsert in `apply_planned_operation`, reconcile every cardinality-many relationship the operation carries as an explicit **replace-set** against the saved node via `_replace_relationship_set` — enforced, not assumed of the upsert, because whether the upsert mutation replaces or merges a relationship list cannot be verified offline (AD007); `peers: []` under `cardinality: "many"` means empty the set and the replace-set acts on it. **The reconciliation runs against a node re-read from the destination (AD054).** The node this step holds was built locally from the write payload, and a relationship manager sets `self.initialized = data is not None` (`.venv/…/infrahub_sdk/node/relationship.py:264`) with `fetch()` returning immediately once initialized (`:286-299`) — so without the T042 re-read, `peer_ids` **is** the desired set, `compare_lists` finds nothing to remove, and the whole reconciliation is a guaranteed no-op that removes nothing against a real destination while passing against a mock — FR-013, FR-028.2, AD038, AD054.
  **Done when**: T051 passes, including its case where the destination's existing peer set differs from
  the plan's and the surplus peers are **removed**.
- [ ] T047 **[AD055 / AD058 / AD062]** [US1] Replace the body of `Potenda.apply_plan` in `infrahub_sync/potenda/__init__.py` — load the new artifact, run the five verification checks plus the write-surface check **before any write**, passing `write_surface_missing_on=<adapter class name>` or `None` rather than a boolean (AD058), and execute operations in stored order with no re-sorting and no recomputation.
  **This task pins the apply record's one named home (AD062).** `RunFile.KEYS` is a closed tuple and this outcome declares the `cache/` layer unchanged, so the record lives under three named keys of the run file's already-free-form `summary` mapping (`infrahub_sync/cache/sidecars.py:73`, `:76`) — no persisted schema other code reads is extended. Every task that reads or writes it references this decision: T055, T056, T059, T065.
    1. `summary["applied_operations"]` — the **ordered** applied identifiers (FR-020, whose final element is FR-025's last-applied pointer).
    2. `summary["skipped_delete_operations"]` — the identifiers of deletes not executed, in stored order.
    3. `summary["skipped_delete_count"]` — their count.
  **A delete-bearing plan ends `applied`, not `failed` (AD055).** Collect `delete` operations rather than stopping; apply every non-delete; then, when the count is non-zero, emit **one operator-visible warning on the run's log stream naming the count** and record the run `applied`. Not executing a delete is a designed limitation of this release — DBR-010 puts applying deletes out of scope — so a run that behaves exactly as designed must not be reported as broken. Under the engine's fallback flag set (`infrahub_sync/potenda/__init__.py:92-93`) any destination holding mapped objects absent from the source yields deletes, so this is the ordinary case, not an exception. **No new run state is introduced.**
  **Assert the knowability invariant in the loop**, because it is what DBR-016 actually protects: on a completed apply, `set(applied) | set(skipped)` equals the plan's identifier set and `len(applied) + count == manifest.operations_count`. That is what makes the reviewed set minus the applied set a **recorded value** rather than an inference — the difference between a disclosed skip and a silent one.
  A destination rejection or transport failure stops at that operation, keeps what was written, records the three keys as they stand, and fails naming the operation identifier, the underlying error and the next action (AD027, AD059) — which is also what keeps a partial apply distinguishable from a completed one, since the invariant above does not hold for it. An empty plan is a successful no-op but verification still runs first — FR-012, FR-016, FR-017, FR-020, FR-022, FR-023, FR-025, AD055, AD058, AD062.
...
- [ ] T051 [P] **[AD038 / AD054 / Trap 4b]** [US5] Write `tests/adapters/test_infrahub_planned_write.py` (replace-set cases) — after the upsert, existing-only peers are removed and new-only peers added; an empty `peers` list under `cardinality: "many"` empties the set; when the upsert already replaced, the reconciliation is a no-op with empty `existing_only`/`new_only`. **The load-bearing case (AD054): the destination's existing peer set must be seeded to differ from the plan's, and the surplus peers asserted removed.** Assert the re-read happened — the relationship manager was fetched before `peer_ids` was read — rather than inferring it from the result. Without this, the whole reconciliation is a no-op that passes: a node built from the write payload reports `self.initialized = data is not None` (`.venv/…/infrahub_sdk/node/relationship.py:264`), `fetch()` returns immediately (`:286-299`), and `peer_ids` *is* the desired set, so `compare_lists` finds nothing to remove. **The test must fail if the re-read is removed** — FR-013, FR-028.2, AD038, AD054.
...
- [ ] T054 [P] **[AD055]** [US4] Write `tests/adapters/test_infrahub_planned_write.py` (**SC-007 local half**) — a plan fixture containing at least one delete alongside creates and updates. Assert, against a recording fake destination: every non-delete operation is applied; the delete is **never dispatched**; the run ends **`applied`**; `summary["skipped_delete_count"]` equals the plan's delete count; `summary["skipped_delete_operations"]` holds exactly those identifiers in stored order; and a captured warning at operator-visible level **names that count**. **Assert the knowability invariant** — `set(summary["applied_operations"]) | set(summary["skipped_delete_operations"])` equals the plan's full identifier set, and the two lengths sum to `manifest.operations_count` — because that, not the run state, is what DBR-016 protects: the reviewed set minus the applied set must be a recorded value, not an inference. **A run state of `failed` must FAIL this test**, asserted explicitly: not executing a delete is a designed limitation of this release, and reporting a designed limitation as a fault is the defect AD055 corrects. Add a **second** case for the class that does still fail: an operations line whose action is outside `ACTIONS` is refused before any write, zero operations dispatched, run `failed`, message naming the identifier, the action found, `ACTIONS` and the next action — FR-016, FR-017, SC-007, AD055, AD059.
- [ ] T055 [P] **[AD062]** [US1] Write `tests/adapters/test_infrahub_planned_write.py` (apply-loop cases) — an adapter without `apply_planned_operation` fails **before any write** with an error naming the adapter class, which the verifier can only produce because it now receives that name rather than a boolean (FR-023, AD058); `summary["applied_operations"]` is an **ordered** sequence and FR-025's last-applied pointer is its final element; a rejection mid-plan stops there, keeps prior writes, records the three summary keys as they stand, and names the failing operation identifier and the next action (AD027, AD059); an empty plan applies as a successful no-op after verification has run, with `skipped_delete_count` `0` (FR-022, AD033); stored order is executed exactly. **Read every assertion back from `run.json`'s `summary` mapping by name (AD062)** — not from an in-memory return value — so the record's pinned home is what is actually tested and a later relocation trips here — FR-012, FR-020, FR-022, FR-023, FR-025, AD058, AD062.
- [ ] T056 [P] [US1] Write the **SC-005** evidence test spanning `tests/plan/test_review.py` and `tests/adapters/test_infrahub_planned_write.py` — capture the operation-identifier set from per-object review output of a stored plan, apply the same plan against a recording fake destination, and assert the review-side set equals the FR-020 record on the run result, per operation and in the same order. **The fixture plan MUST contain no delete operation**, and the test asserts that precondition before comparing: a delete is reviewed but never applied (FR-016, FR-017), so its identifier lands in `summary["skipped_delete_operations"]` rather than `summary["applied_operations"]`, and an order-sensitive equality over a delete-bearing plan fails for a reason that has nothing to do with SC-005. Read the apply-side set from `summary["applied_operations"]` by name (AD062). T054 is where a delete-bearing plan is exercised — FR-003, FR-020, SC-005, AD062.
- [ ] T081 [P] **[AD045a / AD054 / Trap 4]** [US1] Write `tests/plan/test_apply_conformance.py` — a local conformance harness asserting **the mutation the SDK renders**, so the class of defect AD042 was is caught offline instead of only by the deferred live criteria.
  ...
  **Build it against a real `InfrahubNodeSync` constructed from a committed `NodeSchemaAPI` fixture** under `tests/data/`, covering create, update and a relationship-bearing kind mirroring the qualified configuration — including at least one kind whose `human_friendly_id` **crosses a relationship**, so the second arm of AD051's per-component rule is exercised rather than merely declared. Three assertions:
    1. For every operation, the **rendered mutation input** carries `id` or `hfid`. An operation whose payload was built from `source_attrs` alone must fail this — that is the AD042 regression detector, and it is the assertion the earlier form could not make.
    2. The replace-set reconciliation is issued for every cardinality-many relationship, including `peers: []`, **and it re-reads the destination peer set before comparing** (AD054). Seed an existing peer set that differs from the plan's and assert the surplus is removed; the assertion must fail if the re-read is removed.
    3. Applying the same operation twice yields exactly **one** `create` invocation with `allow_upsert=True` — convergence measured at the mutation, which is the only place it is observable offline.
  The harness is not a substitute for SC-002, SC-003 or SC-008, which stay deferred, and it does not close the recorded risk that the SDK cannot form a nested `hfid` client-side from a resolved id (V39, AD051) — FR-013, FR-028.4, AD038, AD042, AD045, AD051, AD054.
  **Done when**: all three assertions pass, and each is demonstrated to fail when its behavior is
  reverted — a payload built from `source_attrs` alone fails assertion 1; removing the re-read fails
  assertion 2; issuing a second create fails assertion 3. A harness whose assertions still pass against
...
- [ ] T058 **[AD056 / AD057 / AD058 / AD061]** [US1] Add **`--from-plan <run-id>`** (a `str | None` option that **takes the run identifier as its value**), `--detail` and `--kind` to the existing `diff` command in `infrahub_sync/cli.py`.
...
- [ ] T059 **[AD055 / AD059 / AD062]** [US2] Rewire the existing `apply` command in `infrahub_sync/cli.py` — refuse **before constructing anything** when the named run does not exist or holds no plan artifact, naming the run identifier, the expected artifact path, **the run identifiers that do exist for that sync** and the next action, and creating no run directory (AD026, AD059); run the five checks plus the write-surface check before any destination write, passing the adapter's **name** (AD058); on refusal record `failed` with `summary["applied_operations"]` as a present, **empty** list and `summary["skipped_delete_count"]` as `0` rather than absent fields (AD062); a run already at `applied` may be applied again and verification still runs (AD033). **A delete-bearing plan exits 0 and records `applied`** with a non-zero skipped-delete count and the warning naming it — the command must not translate a skipped delete into a non-zero exit (AD055) — FR-009, FR-016, FR-017, FR-020, FR-023, AD055, AD059, AD062.
...
| FR-008 | T058, T061, T062, T063, T068, T069, T070, T089, T090 |
...
| SC-007 | T054 (local half: run `applied`, skip count and identifiers recorded, warning names the count), T065 (CLI exit 0 on a delete-bearing plan), T078 (live half) — re-derived per AD055 | local + **integration (live half deferred)** |
...
- **No change to the live `sync` write path's peer behavior.** The zero-match and multi-match refusals
  ...
  (AD048). The one deliberate exception is the shared `_replace_relationship_set` helper, whose
  fetch-before-read ordering is corrected on both callers (AD054), because a helper cannot be a
  replace-set on one path and additive on the other.
