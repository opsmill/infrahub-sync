...
AD001–AD085 in the spec's Clarifications are treated as settled input throughout. Where this plan had
...
| **AD075** | The replace-set reconciliation gains an explicit **flush** after the loop, because the peer-set editors are purely local and an unflushed reconciliation is discarded. The test observable moves onto **the issued destination write carrying the reconciled peer list**. The flush form is amended by AD085 |
...
├── spec.md                                  # input (AD001–AD085 settled)
├── plan.md                                  # this file
...
- Cardinality-many relationships are then reconciled as an explicit **replace-set** against the saved
  ...
  **no `save`**: `add()` and `remove()` only mutate the in-memory peer list and set an update flag, issuing
  ...
  (V40). The sequence therefore ends with an explicit flush — `node.update(do_full_update=True)` after the
  ...
  instead; and `do_full_update=True`, **not** a plain `node.save()` (AD085), because a plain save renders
  with unmodified fields stripped and that stripping drops an **emptied** peer set — the create payload
  wrote `[]` for the same field, the rendered value equals it, and the key is popped, so `peers: []` never
  reaches the destination. `do_full_update=True` renders with the stripping off, the manager renders the
  full peer list — which is what makes the write a replace — and `id` is still rendered, so the update
  targets the reconciled node (V40, V42). This is the same shape as the pre-existing code, whose flush lives in its *caller*
...
destination**: the step 7e flush — `node.update(do_full_update=True)` on the reconciled node — rendered as
an **update** carrying that peer list, not as a second upsert, and carrying it for an **emptied** set too
(AD075, AD085), and that two applies of one operation render
...
| *(no SC — offline conformance, AD045a as rebuilt by AD054 and sharpened by AD065/AD067/AD068/AD075)* | **Rendered-mutation** conformance against a real `InfrahubNodeSync` built from a **committed `NodeSchemaAPI` fixture**: the rendered mutation input carries `id` or `hfid` (`.venv/…/infrahub_sdk/node/node.py:295-298`, `:1843-1846`) — asserted outright for an all-direct HFID, and as a `xfail(strict=True)` for a relationship-crossing HFID, which cannot render keyed today (AD067); the replace-set reconciliation **issued a destination read** for the relationship before reading the peer set it compares against (AD065); the reconciled peer set was **issued to the destination** by the flush — `node.update(do_full_update=True)` on the reconciled node, rendering as an **update** carrying that peer list rather than a second upsert, for an **emptied** set as well as a non-empty one (AD075, AD085); two applies of one operation render **byte-identical** inputs (AD068). Not a criterion of its own — it is the offline half of the assurance SC-002, SC-003 and SC-008 carry, so an AD042-class defect is caught without waiting for the deferred live run. Its earlier form asserted the assembled `data` against a wholly mocked SDK, where two of the three assertions cannot fail for the right reason | local (real node, committed schema fixture) |
