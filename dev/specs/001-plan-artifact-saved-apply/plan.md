...
- Cardinality-many relationships are then reconciled as an explicit **replace-set** against the saved
  ...
  **The re-read is a mechanism, not an ordering (AD065).** `fetch()` carries its own
  ...
  would then land on the fetched node's manager while the flush below saves the node whose manager is still
  the payload-built one — carrying no update flag, so the stripping pops it and the update goes out with no
  relationship at all. Forcing the manager cold is that same scoped read plus the write-back the invariant
  ...
  **no `save`**: `add()` and `remove()` only mutate the in-memory peer list and set an update flag, issuing
  ...
  every cardinality-many relationship, once, not one save per relationship, **on the same node object the
  reconciliation ran against; the node that is saved must be the node whose manager was reconciled**. Plain, **not**
...
destination**: the step 7e flush — a plain `node.save()` on the reconciled node — rendered as an **update**
carrying that peer list, not as a second upsert (AD075), and that two applies of one operation render
