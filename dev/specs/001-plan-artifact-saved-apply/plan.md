...
| SC-016 (live half) | **Not produced — refused as a fixture precondition, and it cannot be satisfied on this destination schema.** `test_an_ambiguous_peer_refuses_the_operation` errors in setup: seeding a genuinely ambiguous peer needs a referenced kind whose uniqueness constraints do not cover the components the resolver filters on, and **every one of the 20 kinds this configuration touches declares one that does** (V30). The destination answered the clone with `Violates uniqueness constraint 'device-name'`, HTTP 422. The offline half (zero-match and multi-match refusal messages) is unaffected and still passes. Not weakened, not skipped, not mocked |
