...
If no ticket ID is found in the arguments, first run
`git rev-parse --abbrev-ref HEAD`:

- If the current branch matches `-(infp|ifc)-[0-9]+$`, extract its ticket
  suffix and offer it as an explicit default:

  > "Please provide a Jira or JPD reference for this feature. The current branch is `<branch>` — reply `<its-ticket>` to continue that feature, or give a different ticket (e.g., `infp-646` or `ifc-2140`) to start a new one."
...
After normalizing the ticket ID to lowercase, run
`git rev-parse --abbrev-ref HEAD`. If the current branch ends in
`-<normalized-ticket>`, output:

> "✓ Reusing feature branch: <branch>"

and stop — do not generate a short name and do not call the script.
...
- Run this script at most once per feature; skip when Step 2 reuses the branch
