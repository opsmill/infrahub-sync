...
metadata:
  author: opsmill
  source: infrahub-sync repo — local shim; the opsmill extension invokes this skill from prep/implement/auto but does not ship it. Remove once the extension provides it.
user-invocable: true
...
5. Soft-check the branch name. If it does not match one of the speckit
   feature-branch patterns — sequential `^[0-9]{3,}-` or timestamp
   ...
   the hard guards above cover the dangerous cases.)
