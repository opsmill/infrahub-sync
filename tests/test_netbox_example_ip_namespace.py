...
PACKAGE_PATH = EXAMPLE_DIR / "package.yml"
...
def test_the_package_carries_the_same_configuration_as_the_standalone_file() -> None:
    """`package.yml` embeds `config.yml` as its `configuration` body, so both must agree.

    The two are edited separately and only the package reaches the service, so a mapping
    change applied to one and not the other would ship a package that does not match the
    example a reader is told to copy.
    """
    assert yaml.safe_load(CONFIG_PATH.read_text()) == yaml.safe_load(PACKAGE_PATH.read_text())["configuration"]
