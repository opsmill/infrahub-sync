...
from infrahub_sync.service.liveness import LivenessPolicy, RunLivenessReconciler, select_cancellable_execution
from infrahub_sync.service.orchestration import CancellationResult, Observation, PoolStatus, PoolWorker, Submission
...
def test_admission_ttl_and_prefect_query_define_liveness_formulae(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from infrahub_sync.service.liveness import LivenessPolicy
...
def test_prefect_query_seconds_refuses_out_of_domain_values(monkeypatch: pytest.MonkeyPatch, value: str) -> None:
    monkeypatch.setenv("INFRAHUB_SYNC_RUN_ADMISSION_TTL_SECONDS", "300")
    with pytest.raises(ValueError, match=r"^service liveness settings are invalid$"):
...
def test_admission_ttl_refuses_oversized_digits_with_fixed_unchained_error(
    ...
    with pytest.raises(ValueError, match=r"^service liveness settings are invalid$") as caught:
...
def test_custom_named_terminal_prefect_states_interrupt_a_claimed_execution(
    ...
    from infrahub_sync.service.orchestration import PrefectOrchestration
...
def test_pool_parsing_reports_a_ready_worker_when_freshness_is_exactly_at_the_bound() -> None:
    ...
    from infrahub_sync.service.orchestration import PrefectOrchestration
    from infrahub_sync.service.service import _service_status  # noqa: PLC2701 - public status behavior test.
...
def test_duplicate_worker_uuid_snapshot_is_unavailable_and_cannot_drive_reconciliation(
    ...
    from infrahub_sync.service.orchestration import PrefectOrchestration
    from infrahub_sync.service.service import _service_status  # noqa: PLC2701 - public boundary behavior test.
...
def test_public_worker_status_refuses_count_and_availability_invariant_violations(payload: dict[str, object]) -> None:
    ...
    from infrahub_sync.service.models import WorkerStatusResource
...
def test_request_time_reconciliation_terminalizes_each_pending_link(tmp_path) -> None:
    ...
    app = create_app(RunService(projection, orchestration), _Resolver(), reconciler=reconciler)  # type: ignore[arg-type]
