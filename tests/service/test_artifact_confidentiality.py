...
class _StubOrchestration:
    """The read routes under test never submit, so observation is all this has to answer."""
...
def test_the_run_resource_does_not_carry_internal_references(
    ...
    assert response.status_code == 200, response.text
    assert [item["artifact_id"] for item in response.json()["artifact_refs"]] == [PLAN_ARTIFACT_ID]
