...
def test_a_valid_member_name_is_still_delivered() -> None:
    """Preservation: an accepted member name is carried through unchanged."""
    snapshot = capabilities_module._build_schema_snapshot(
        {"InfraDevice": _node(attributes=[{"name": "mtu_bytes", **_TEXT}], relationships=[])}
    )

    assert set(snapshot["InfraDevice"]["attributes"]) == {"mtu_bytes"}
