...
INFRAHUB_SYNC_API_TOKEN=$(setting INFRAHUB_SYNC_SERVICE_BEARER_TOKENS | sed 's/.*"token": *"//;s/".*//')
...
row_secrets() {
    ...
    for name in $SECRET_SETTINGS; do
        value=$(setting "$name")
        case $value in
            ''|*'${'*) fail "$name holds no generated value for this run to sweep for" ;;
        esac
        printf '%s\n' "$value" >> "$WORK/canaries"
    done
    ...
    while read -r canary; do
        ...
        for target in $swept; do
            if grep -qF -- "$canary" "$target" 2>/dev/null; then
