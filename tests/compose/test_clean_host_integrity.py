...
# ---------------------------------------------------------------------------
# Reading one stored object: the get, the stream, and the close
# ---------------------------------------------------------------------------
class RawStream:
    """What botocore hands back from a body's context manager.

    `StreamingBody.__enter__` returns `self._raw_stream`, a urllib3
    `HTTPResponse`, which carries no `iter_chunks`. A fake whose `__enter__`
    returned the wrapper would be the one shape this cannot go wrong in.
    """


class FakeBody:
    """One body, with botocore's own wrapper, entry and close behaviour."""
...
class FakeStore:
    """The one call this boundary makes, and what it was asked for."""
    ...
    def get_object(self, **query: str) -> dict[str, FakeBody]:
        self.requested.append((query["Bucket"], query["Key"]))
        return {"Body": self._body}


def test_a_stored_object_is_digested_raw_and_its_wrapper_closed(durable_state: ModuleType) -> None:
    """Entering a body hands back the raw stream, which cannot stream chunks.

    So the wrapper is what has to be held and closed. The chunking here is
    uneven because the digest covers the object's bytes and not the boundaries
    the read happened to arrive in.
    """
    key = "runs/r-1/manifest.json"
    chunks = [b'{"checksum": ', b'"abcd"}']
    body = FakeBody(chunks)
    store = FakeStore(body)

    line = durable_state.stored_object_line(store, "infrahub-sync", key)

    assert line == f"object {key} {sha256(b''.join(chunks)).hexdigest()}"
    assert store.requested == [("infrahub-sync", key)]
