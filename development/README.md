...
`netbox.seed` takes `--dataset` (default `seed`; the task structure leaves room for a second,
`demo`, dataset). Starting any dataset resets the database first, so re-running `netbox.seed`
is safe to repeat. Export the printed values as `NETBOX_URL` and `NETBOX_TOKEN`, point
